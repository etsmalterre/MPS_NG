...
- **Polymorphic `ged` rows**: a single `ged` row can be linked to multiple parents at once (a GOTS cert is shared by the fournisseur / client / sous-traitant commandes in the same chain). The discriminator is `IDtype_doc`, **not** which `IDcommande_*` columns are zero. **Never** filter by `IDcommande_client = 0 AND IDcommande_sous_traitant = 0` — that silently hides shared legacy rows. Whitelist by `IDtype_doc` instead. Per-screen whitelists live in the route files (see `commandes-fil.ts`).
...
- **Ennoblisseur `prix` is auto-computed**: `apps/api/src/lib/pricing-sst.ts` ports the legacy `CalculTarifSST` — base tariff from `tranche_tarif_ennoblissement` + hardcoded MATEL (sst=9) / ESAT (sst=89) multipliers on Lavage (285) / BNO-PAT (302). `recalcLignePrix(ligneId)` fires on every écru link/unlink and writes `prix`. SSTs without catalog rows keep manual entry — detail endpoint surfaces `auto_pricing_enabled` to gate the frontend lock. Breakdown via `GET /:cId/lignes/:lId/prix-breakdown`. Full algo in memory `project_pricing_calcultarifsst.md`.
